...
def _convert_x(mode: str, value: float, unit: Union[str, None]) -> float:
    ...
    if mode in {TGA, DSC}:
        ...
        if key in {"h", "hr", "hour", "hours"}:
            return value * 60.0
    elif mode in {SS_CURVE, ADHESION}:
        if key == "mm":
            return value
        if key in {"µm", "um"}:
            return value / 1000.0
...
def _convert_y(mode: str, value: float, unit: Union[str, None]) -> tuple[float, Union[str, None]]:
    ...
    elif mode in {SS_CURVE, ADHESION} and key == "n":
        return value, "N"
...
def _curve_from_values(
    ...
    if profile.measurement_type == TEMPERATURE_LOGGER:
        return CurveData(
            temperatures=(), mass_mg=(), weight_percent=(),
            logger_time_min=x, logger_temperature_c=y, **common,
        )
    if profile.measurement_type == SS_CURVE:
        return CurveData(
            temperatures=(), mass_mg=(), weight_percent=(),
            extension_mm=x, force_n=y, **common,
        )
    if profile.measurement_type == ADHESION:
        return CurveData(
            temperatures=(), mass_mg=(), weight_percent=(),
            distance_mm=x, force_n=y, **common,
        )
