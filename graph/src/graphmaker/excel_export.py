...
class AnalysisResultTable:
    ...
    def validate(self) -> None:
        if not self.headers:
            raise ValueError("解析結果表のヘッダーがありません。")
        if any(len(row) != len(self.headers) for row in self.rows):
            raise ValueError("解析結果表の列数が一致していません。")


def export_excel(
    curves: Sequence[Union[CurveData, ProcessedCurveData, ParticleSizeProcessedData, DerivedCurveData, DisplaySeries]],
    plot_state: PlotState,
    output_path: Union[Path, str],
    analysis_table: Union[AnalysisResultTable, None] = None,
) -> Path:
    if not curves:
    ...
    if analysis_table is not None:
        analysis_table.validate()
        result_start_col = start_col + 2
        for offset, header in enumerate(analysis_table.headers):
            column = result_start_col + offset
            data_sheet.cell(row=DATA_HEADER_ROW, column=column, value=header)
            data_sheet.column_dimensions[get_column_letter(column)].width = DATA_COLUMN_WIDTH
        for row_offset, values in enumerate(analysis_table.rows, start=DATA_START_ROW):
            for column_offset, value in enumerate(values):
                data_sheet.cell(
                    row=row_offset,
                    column=result_start_col + column_offset,
                    value=value,
                )
