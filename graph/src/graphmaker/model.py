...
def path_key(path: Union[Path, str]) -> str:
...
def normalize_measurement_type(value: str) -> str:
    ...
    canonical = {
        ...
        TEMPERATURE_LOGGER.upper(): TEMPERATURE_LOGGER,
        "TEMPERATURE_LOGGER": TEMPERATURE_LOGGER,
        SS_CURVE.upper(): SS_CURVE,
        "SS_CURVE": SS_CURVE,
        ADHESION.upper(): ADHESION,
        "ADHESION": ADHESION,
    }.get(normalized)
...
class CurveData:
    ...
    source_heat_flow_header: Union[str, None] = None
    legend_name: Union[str, None] = None
    ...
    source_particle_diameter_header: Union[str, None] = None
    source_volume_frequency_header: Union[str, None] = None
    logger_time_min: tuple[float, ...] = ()
    logger_temperature_c: tuple[float, ...] = ()
    extension_mm: tuple[float, ...] = ()
    force_n: tuple[float, ...] = ()
    distance_mm: tuple[float, ...] = ()
    sample_mass_mg: Union[float, None] = None
    heating_rate_c_min: Union[float, None] = None
    import_provenance: Union[ImportProvenance, None] = None
    ...
    def __post_init__(self) -> None:
        ...
        elif self.measurement_type == TEMPERATURE_LOGGER:
            count = len(self.logger_time_min)
        elif self.measurement_type == SS_CURVE:
            count = len(self.extension_mm)
        elif self.measurement_type == ADHESION:
            count = len(self.distance_mm)
        else:
        ...
        if self.measurement_type == TGA:
            ...
            self.measurement_type == TEMPERATURE_LOGGER
            and len(self.logger_temperature_c) != count
        ):
            raise ValueError("All temperature-logger curve arrays must have the same length")
        elif self.measurement_type in {SS_CURVE, ADHESION} and len(self.force_n) != count:
            raise ValueError("All force curve arrays must have the same length")
    ...
    def plot_y(self) -> tuple[float, ...]:
        ...
        if self.measurement_type == TEMPERATURE_LOGGER:
            return self.logger_temperature_c
        if self.measurement_type in {SS_CURVE, ADHESION}:
            return self.force_n
    ...
    def plot_x(self) -> tuple[float, ...]:
        ...
        if self.measurement_type == TEMPERATURE_LOGGER:
            return self.logger_time_min
        if self.measurement_type == SS_CURVE:
            return self.extension_mm
        if self.measurement_type == ADHESION:
...
def default_axis_range(measurement_type: str) -> AxisRange:
    ...
    if mode == TEMPERATURE_LOGGER:
        return AxisRange(0.0, 60.0, 0.0, 100.0)
    if mode == SS_CURVE:
        return AxisRange(0.0, 100.0, 0.0, 10.0)
    if mode == ADHESION:
        return AxisRange(0.0, 100.0, 0.0, 10.0)
...
class PlotState:
    ...
    axis_range: Union[AxisRange, None] = None
    auto_axes: bool = True
    measurement_type: str = TGA
