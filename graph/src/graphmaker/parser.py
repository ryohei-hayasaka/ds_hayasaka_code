...
TEMPERATURE_LOGGER_REQUIRED_HEADERS = ("Time_min", "Temperature_C")
SS_CURVE_REQUIRED_HEADERS = ("Extension_mm", "Force_N")
ADHESION_REQUIRED_HEADERS = ("Distance_mm", "Force_N")
...
class TemperatureLoggerDataError(MeasurementDataError):
    pass


class SsCurveDataError(MeasurementDataError):
    pass


class AdhesionDataError(MeasurementDataError):
    pass
...
def load_tga_csv(path: Union[Path, str]) -> CurveData:
...
def load_dsc_csv(path: Union[Path, str]) -> CurveData:
...
def load_ir_csv(path: Union[Path, str]) -> CurveData:
...
def load_uvvis_csv(path: Union[Path, str]) -> CurveData:
...
def load_gpc_csv(path: Union[Path, str]) -> CurveData:
...
def load_particle_size_csv(path: Union[Path, str]) -> CurveData:
...
def load_temperature_logger_csv(path: Union[Path, str]) -> CurveData:
    file_path, reader = _open_csv(
        path, TEMPERATURE_LOGGER_REQUIRED_HEADERS, TemperatureLoggerDataError
    )
    fields = reader.canonical_fields  # type: ignore[attr-defined]
    times: list[float] = []
    temperatures: list[float] = []
    for row in reader:
        ...
        line_number = reader.line_num
        times.append(
            _number(row, fields["Time_min"], "Time_min", file_path, line_number, TemperatureLoggerDataError)
        )
        temperatures.append(
            _number(row, fields["Temperature_C"], "Temperature_C", file_path, line_number, TemperatureLoggerDataError)
        )
    _validate_increasing(times, file_path, TemperatureLoggerDataError, "時間")
    return CurveData(
        path=file_path.resolve(),
        display_name=file_path.stem,
        temperatures=(),
        mass_mg=(),
        weight_percent=(),
        measurement_type=TEMPERATURE_LOGGER,
        logger_time_min=tuple(times),
        logger_temperature_c=tuple(temperatures),
    )


def load_ss_curve_csv(path: Union[Path, str]) -> CurveData:
    file_path, reader = _open_csv(path, SS_CURVE_REQUIRED_HEADERS, SsCurveDataError)
    fields = reader.canonical_fields  # type: ignore[attr-defined]
    ...
    for row in reader:
        if not row or all(value is None or not str(value).strip() for value in row.values()):
            continue
        line_number = reader.line_num
        extension.append(_number(row, fields["Extension_mm"], "Extension_mm", file_path, line_number, SsCurveDataError))
        force.append(_number(row, fields["Force_N"], "Force_N", file_path, line_number, SsCurveDataError))
    _validate_increasing(extension, file_path, SsCurveDataError, "伸び")
    return CurveData(
        path=file_path.resolve(), display_name=file_path.stem,
        temperatures=(), mass_mg=(), weight_percent=(), measurement_type=SS_CURVE,
        extension_mm=tuple(extension), force_n=tuple(force),
    )


def load_adhesion_csv(path: Union[Path, str]) -> CurveData:
    file_path, reader = _open_csv(path, ADHESION_REQUIRED_HEADERS, AdhesionDataError)
    fields = reader.canonical_fields  # type: ignore[attr-defined]
    distance: list[float] = []
    force: list[float] = []
    for row in reader:
        if not row or all(value is None or not str(value).strip() for value in row.values()):
            continue
        line_number = reader.line_num
        distance.append(_number(row, fields["Distance_mm"], "Distance_mm", file_path, line_number, AdhesionDataError))
        force.append(_number(row, fields["Force_N"], "Force_N", file_path, line_number, AdhesionDataError))
    _validate_increasing(distance, file_path, AdhesionDataError, "移動距離")
    return CurveData(
        path=file_path.resolve(), display_name=file_path.stem,
        temperatures=(), mass_mg=(), weight_percent=(), measurement_type=ADHESION,
        distance_mm=tuple(distance), force_n=tuple(force),
    )


def _validate_increasing(values, file_path, error_type, label: str) -> None:
    if len(values) < 2:
        raise error_type(f"{label}データ点が不足しています（2点以上必要です）: {file_path.name}")
    for index in range(1, len(values)):
        if values[index] <= values[index - 1]:
            reason = "同じX値があります" if values[index] == values[index - 1] else "X値の順序が不正です"
            raise error_type(f"{file_path.name} の{index + 2}行目: {label}の{reason}。")
