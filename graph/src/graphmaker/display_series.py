...
from .new_mode_analysis import DerivedCurveData
...
def to_display_series(
    curve: Union[CurveData, ProcessedCurveData, ParticleSizeProcessedData, DerivedCurveData, DisplaySeries],
) -> DisplaySeries:
    if isinstance(curve, DisplaySeries):
        return curve

    processed_types = (ProcessedCurveData, ParticleSizeProcessedData, DerivedCurveData)
    ...
    if mode == UV_VIS:
        x_values = curve.display_x if isinstance(curve, ProcessedCurveData) else source.wavelengths_nm
        y_values = curve.display_y if isinstance(curve, ProcessedCurveData) else source.uvvis_absorbance
        normalized = isinstance(curve, ProcessedCurveData) and curve.status == NORMALIZED
        return _make(
            source,
            x_values,
    ...
    if mode == TEMPERATURE_LOGGER:
        return _make(
            source,
            source.logger_time_min,
            source.logger_temperature_c,
            "Time (min)",
            "Temperature (°C)",
            "Time_min",
            "Temperature_C",
    ...
    if mode in {SS_CURVE, ADHESION}:
        if isinstance(curve, DerivedCurveData):
            return _make(
                source,
                curve.display_x,
                curve.display_y,
    ...
    x_values = curve.display_x if isinstance(curve, ProcessedCurveData) else source.retention_times_min
    y_values = curve.display_y if isinstance(curve, ProcessedCurveData) else source.ri_signal_mv
    normalized = isinstance(curve, ProcessedCurveData) and curve.status == NORMALIZED
    return _make(
        source,
        x_values,
        y_values,
        "Retention time (min)",
        "Normalized RI response" if normalized else "RI response (mV)",
        "RetentionTime_min",
        "RI_mV",
        metadata=metadata,
        normalized=normalized,
...
def _default_axis_titles(measurement_type: str) -> tuple[str, str]:
    return {
        ...
        TEMPERATURE_LOGGER: ("Time (min)", "Temperature (°C)"),
        SS_CURVE: ("Strain (%)", "Stress (MPa)"),
        ADHESION: ("Distance (mm)", "Adhesive force (N/25 mm)"),
    }[measurement_type]
