...
def plot_bounds(width: float, height: float) -> tuple[float, float, float, float] | None:
...
class MeasurementPlotCanvas(tk.Canvas):
    def __init__(self, master, **kwargs):
        ...
        self._dsc_session: DscAnalysisSession | None = None
        self._dsc_curve: CurveData | None = None
        self._dsc_selection: DscFourPointSelection | None = None
        self._annotation_visibility: dict[str, bool] = {}
        self._normalization_wavenumber: float | None = None
        self._normalization_diameter_um: float | None = None
        self.bind("<Configure>", lambda _event: self.after_idle(self.redraw))

    def temperature_from_canvas_point(self, canvas_x: float, canvas_y: float) -> float | None:
        return self.x_from_canvas_point(canvas_x, canvas_y)

    def x_from_canvas_point(self, canvas_x: float, canvas_y: float) -> float | None:
    ...
    def set_plot(
        self,
        curves: Sequence[CurveData | ProcessedCurveData | DisplaySeries],
        axis_range: AxisRange,
        measurement_type: str = TGA,
        dsc_session: DscAnalysisSession | None = None,
        dsc_curve: CurveData | None = None,
        dsc_selection: DscFourPointSelection | None = None,
        annotation_visibility: dict[str, bool] | None = None,
        normalization_wavenumber: float | None = None,
    ...
    def _draw_dsc_background(
        ...
        if melting is not None and self._visible("enthalpy_area"):
            upper = [
                (px(x), py(y))
                for x, y in zip(
                    melting.integration_temperatures,
                    melting.integration_heat_flow,
                    strict=True,
                )
        ...
        if tg is not None:
            if self._visible("tg_smoothed") and self._dsc_curve is not None:
                ...
                if len(curve.temperatures) == len(tg.smoothed_heat_flow):
                    points = []
                    for x, y in zip(curve.temperatures, tg.smoothed_heat_flow, strict=True):
                        if self._axis_range.x_min <= x <= self._axis_range.x_max:
