...
class ImportProfile:
    ...
    @classmethod
    def from_dict(cls, data: dict[str, object], *, built_in: bool | None = None) -> "ImportProfile":
        payload = dict(data)
...
class ProfileStore:
    ...
    def all(self, measurement_type: str | None = None, *, enabled_only: bool = True) -> tuple[ImportProfile, ...]:
    ...
    def get(self, profile_id: str) -> ImportProfile | None:
    ...
    def duplicate(self, profile_id: str, name: str | None = None) -> ImportProfile:
...
def preview_csv(path: Path | str, profile: ImportProfile | None = None) -> ImportPreview:
    file_path = Path(path)
...
def _row_by_line(rows: list[PreviewRow], line_number: int) -> PreviewRow | None:
    return next((row for row in rows if row.line_number == line_number), None)


def _column_indices(header: PreviewRow | None, profile: ImportProfile) -> dict[str, int]:
...
def _candidate_start_line(
    rows: list[PreviewRow], profile: ImportProfile, header_line: int | None
) -> int:
...
def _resolve_header(rows: list[PreviewRow], profile: ImportProfile) -> int | None:
...
def _convert_x(mode: str, value: float, unit: str | None) -> float:
    key = _unit_key(unit)
    if mode in {TGA, DSC}:
        if key in {"°c", "℃", "c"}:
            return value
...
def _convert_y(mode: str, value: float, unit: str | None) -> tuple[float, str | None]:
    key = _unit_key(unit)
    if mode == TGA:
        if key == "mg":
            return value, "mg"
...
def _read_profiled_data(path: Path, profile: ImportProfile) -> ImportTestResult:
    ...
    canonical_y_unit: str | None = None
...
def _column_description(column: int, header: PreviewRow | None) -> str:
...
def test_import(path: Path | str, profile: ImportProfile) -> ImportTestResult:
    return _read_profiled_data(Path(path), profile)


def load_curve_with_profile(path: Path | str, profile: ImportProfile) -> CurveData:
