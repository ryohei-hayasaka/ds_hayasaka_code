...
class DscAnalysisSettings:
    ...
    tg_range: TemperatureRange | None = None
    tg_pre_range: TemperatureRange | None = None
    tg_post_range: TemperatureRange | None = None
    melt_range: TemperatureRange | None = None
    melt_pre_range: TemperatureRange | None = None
    melt_post_range: TemperatureRange | None = None
...
class MeltingResult:
    ...
    peak_c: float
    end_c: float
    enthalpy_j_g: float | None
    enthalpy_signed_j_g: float | None
    analysis_range: TemperatureRange
    pre_range: TemperatureRange
    post_range: TemperatureRange
    baseline: LineFit
...
class DscRangeSuggestions:
    tg_range: TemperatureRange | None
    tg_pre_range: TemperatureRange | None
    tg_post_range: TemperatureRange | None
    melt_range: TemperatureRange | None
    melt_pre_range: TemperatureRange | None
    melt_post_range: TemperatureRange | None
    warnings: tuple[str, ...] = ()
...
class DscAnalysisSession:
    ...
    tg_result: TgResult | None = None
    melting_result: MeltingResult | None = None
    decision: str = "候補"
    status: str = "未解析"
...
def infer_heating_rate(curve: CurveData) -> float | None:
...
def suggest_dsc_ranges(
    ...
    *,
    ...
    corrected = tuple(
        ...
        for temperature, value in zip(temperatures, smoothed, strict=True)
    )
    noise = _rms(
...
def analyze_melting(curve: CurveData, settings: DscAnalysisSettings) -> MeltingResult:
    ...
    corrected = tuple(
        ...
        for temperature, value in zip(temperatures, heat_flow, strict=True)
    )
    ...
    signed_area = _trapz(
        ...
        tuple(
            value - base
            for value, base in zip(integration_heat_flow, integration_baseline, strict=True)
        ),
    )
...
def _ordered_curve(curve: CurveData) -> tuple[tuple[float, ...], tuple[float, ...]]:
    ...
    pairs = list(zip(curve.temperatures, curve.heat_flow_mw, strict=True))
    delta = curve.temperatures[-1] - curve.temperatures[0]
    if delta < 0:
        pairs.reverse()
...
def _required_range(value: TemperatureRange | None, label: str) -> TemperatureRange:
    if value is None:
...
def _fit_rms(xs: Sequence[float], ys: Sequence[float], line: LineFit) -> float:
    return _rms([value - line.at(x) for x, value in zip(xs, ys, strict=True)])
