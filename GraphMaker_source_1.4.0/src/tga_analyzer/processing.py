...
class ProcessedCurveData:
    ...
    normalization_wavenumber: float | None = None
    warnings: tuple[str, ...] = ()
    blank_corrected_x: tuple[float, ...] | None = None
    blank_corrected_y: tuple[float, ...] | None = None
    ...
    @property
    def heat_flow_unit(self) -> str | None:
        return self.source.heat_flow_unit

    @property
    def source_heat_flow_header(self) -> str | None:
...
def normalize_ir_curve(
    curve_or_x: CurveData | ProcessedCurveData | Sequence[float],
...
def process_ir_curve(
    ...
    available_curves: Mapping[str, CurveData],
    all_series_settings: Mapping[str, SeriesProcessingSettings] | None = None,
) -> ProcessedCurveData:
...
def _process_curve(
    ...
    all_series_settings: Mapping[str, SeriesProcessingSettings] | None,
    mode: str,
) -> ProcessedCurveData:
    series.validate()
...
def _subtract_with_interpolation(
    ...
    output_x: list[float] = []
    output_y: list[float] = []
    for x_value, sample_value in zip(sample_x, sample_y, strict=True):
        if low <= x_value <= high:
...
def _values_matching_x(
    ...
    by_x = {x: value for x, value in zip(source_x, source_values, strict=True)}
